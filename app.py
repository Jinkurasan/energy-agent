...
st.set_page_config(page_title="energy-agent", page_icon="⚡", layout="wide", initial_sidebar_state="collapsed")
...
.title { font-size: 32px; font-weight: 800; color: #9ae6b4; text-align: center; padding: 24px 0 6px; }
.subtitle { color: #718096; text-align: center; font-size: 14px; margin-bottom: 36px; }
.status-ok   { color: #68d391; font-size: 13px; }
.status-warn { color: #f6ad55; font-size: 13px; }
div[data-testid="stExpander"] { background: #1a202c; border: 1px solid #2d3748; border-radius: 8px; }
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="title">⚡ energy-agent</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">エネルギー・環境ビジネス情報収集・配信システム</div>', unsafe_allow_html=True)
st.divider()
...
def check_status() -> dict:
    ...
    return {
        "Anthropic API": bool(ANTHROPIC_API_KEY and not ANTHROPIC_API_KEY.startswith("your")),
...
with col1:
    dry_run = st.checkbox("テストモード（投稿をスキップ）", value=True)
with col2:
...
if run_btn:
    with st.status("パイプラインを実行中...", expanded=True) as status_box:
        try:
            ...
            st.write("🧠 分析・リサーチ部 起動中...")
            summarizer = create_summarizer_agent()
            summary = summarizer.run(f"以下のデータを分析してレポートを作成してください。\n\n{collected}")
            _save_output("2_summary", summary)
            st.write("✅ 分析・レポート作成 完了")

            st.write("✍️ マーケティング部 起動中...")
            ...
            with st.expander("📄 エネルギーレポート全文", expanded=False):
                st.markdown(summary)
